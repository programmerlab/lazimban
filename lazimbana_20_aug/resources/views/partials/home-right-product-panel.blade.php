<div class="col-md-9 col-sm-8">
                                  <div class="add_panel row">
                                      ...
                                      <div class="col-md-6"><img src="{{ asset('storage/files/banner/'.$banner[9]->field_value) }}"></div>
                                      <div class="col-md-6"><img src="{{ asset('storage/files/banner/'.$banner[10]->field_value) }}"></div>
                                  </div>
                                        ...
                                        @foreach($categories as $key => $value) 
                                            @foreach($products as $key2 => $product)
                                                ...
                                                @if($value['id']!=$product->category->parent_id && $key < 1) 
                                                    <li>
                                                        <div class="col-md-3"><a href="{{ url($product->url) }}"><img src="{{ asset('storage/uploads/products/'. $product->photo) }}" alt="{{ $product->img_alt }}" title="{{ $product->img_alt }}"></a></div>
                                  ...
                                  <h3><span>Yeni </span> Gelenler</h3>
                                      <div class="owl-carousel owl-theme">
                                        @foreach($categories as $key => $value)  
                                            @foreach($product_new as $key2 => $product) 
                                                <div class="item">
                                                     <a href="{{ url($product->url) }}"><img src="{{ asset('storage/uploads/products/'. $product->photo) }}" alt="{{ $product->img_alt }}" title="{{ $product->img_alt }}"></a>
